...
\item $\mathcal{E}_j$, containing the extension pairs $(q_{1}', q_{2}')$ of
  length $(j+\tau)$. %, each one with its extension $e$ $> 0$
\end{itemize}
...
and its \$-width $w_\$$ (recall that $w_\$$ is the width of the backward \$-extension of $[b',e')$). %in $B'$.
...
\notaestesa{PB}{questa parte è da rivedere ... cosa si intende per
  irriducibilità?}
%Notice that just $(j+\tau)$-long reads are added to an arc-set at the
%$j$-th iteration.
...
%$B \gets BWT(R)$\;
%$B' \gets	BWT(R')$\;
%$C \gets Count(B)$ \hfill \% Count (C) function\;
...
%\tcp{Initially the arc-set of each read is empty}
...
$\mathcal{P}\gets$ empty file\;
  ...
  %$extendQOnLeft(\mathcal{Q}_j, B, C, \mathcal{Q}_{j+1}, \mathcal{P})$\;
$\Pi(\sigma) \gets 0$ for each $\sigma \in \Sigma \cup \{\$\}$\;

 \ForEach{$\sigma \in \Sigma \cup \{\$\}$}{
      \ForEach{$\langle [b,e),[b',e')\rangle \in \mathcal{Q}_j(\sigma)$}{
	$processLinkedInterval([b, e), [b', e'), B, C, \Pi, F_e, \mathcal{P})$\;
}
}
...
$\Pi(\sigma) \gets 0$ for each $\sigma \in \Sigma \cup \{\$\}$\;
  ...
  %$\mathcal{E}_{j+1} \gets extendQOnRight(\mathcal{P}, \mathcal{E}_j, B', C)$\;
$\mathcal{E}_{j+1} \gets \emptyset$, terminals $\gets \emptyset$
    ...
    \While{$((\mathcal{P} \cup \mathcal{E}_j) \neq \emptyset)$} {
		\If{$(b' < b'_p)$} {
\tcp{The extension consists of a single character}
...
\tcp{We extend a preexisting extension}
    ...
    $\langle [b'_p, e'_p), [b'_s, e'_s), e \rangle \gets$ the next extension pair of $\mathcal{E}_j$\;
                ...
                %checkIfIrreducible($newterminal$)\;
...
%$F_e$, an array of $|\Sigma|$ empty files,\\$\mathcal{P}$,  an empty file}

Initialize a vector $\Pi \gets [0, 0, . . . , 0]$ of $|\Sigma^\$|$ counters\;

 \For{$i \gets 0$ \KwTo  $|\Sigma|$}{
      \ForEach{$\langle [b,e),[b',e')\rangle \in F(\sigma_i)$}{
	$processLinkedInterval([b, e), [b', e'), B, C, \Pi, F_e, \mathcal{P})$\;
% \tcp{check if $[b',e')$ is a prefix-interval}
% 	\If{$[b,e)$ has width $>1$ and $n >0$ $\$$-extensions}{
%           Append $\langle [b', e'), n\rangle$ to file $\mathcal{P}$\;
% 	}
     }
}

\caption{extendQOnLeft($F$, $B$, $C$, $F_e$, $\mathcal{P}$)}
...
\begin{algorithm}
\SetKwInOut{Input}{Input}\SetKwInOut{Output}{Output}
\Input{$\mathcal{P}$, the sorted list of the prefix-intervals of a given length $l$,\\$\mathcal{E}$, the sorted list of the extension pairs of dimension $l$,\\$B'$, the BWT of the reversed set $R'$, \\$C$, count vector for $B'$}
...
Initialize vector $\Pi \gets [0, 0, \ldots , 0]$ of $|\Sigma^\$|$ integers\;
    ...
    $newterminal \gets \emptyset$

    $\langle [b', e'), n \rangle \gets$ getFirst$(\mathcal{P})$\;
    $\langle [b'_p, e'_p), [b'_s, e'_s), e \rangle \gets$ getFirst$(\mathcal{E})$\;

    \While{$((\mathcal{P} \cup \mathcal{E}) \neq \emptyset) \cup (b'_p = NULL)$} {
             % $\langle [b', e'), n \rangle \gets pop((\mathcal{P})$\;
		...
		\Else {
                  $newterminal \gets newterminal \cup processExtPair([b'_p, e'_p), [b'_s, e'_s), e, B', C, \mathcal{E}_e, \Pi)$\;
    $\langle [b'_p, e'_p), [b'_s, e'_s), e \rangle \gets$ getFirst$(\mathcal{E})$\;
...
\caption{extendQOnRight($\mathcal{P}$, $\mathcal{E}$, $B'$, $C$)}
...
\tcp{$\Pi[i]$ is the number of $\sigma_i$ in $B[0,b-1]$, $\sigma_i \in \Sigma^\$$}
...
\ForEach{$i \gets 0$ \KwTo  $|\Sigma^\$|$}{
\tcp{compute the start of the $\sigma_i Q$-interval}
%   $b_e \gets |R|+C(\sigma_i)+ \Pi[i]$\;
   \If{ $\pi[i] > 0$ } {
     \If{ $i = 0$ } {
       \tcp{prefix-Interval}
       Append $\langle [b'_e, e'_e), \pi[i] \rangle$ to $\mathcal{P}$\;
     }
     \Else {
       $b_e \gets C[ i ] + \Pi[i]$\;
       $e_e \gets b_e + \pi[i]$\;
       $b'_e \gets b' + \pi sum$\;
       $e'_e \gets b'_e + \pi[i]$\;
       Append $\langle [b_e, e_e), [b'_e, e'_e)\rangle$ to $F_e(\sigma_i)$\;
     }
...
\caption{processLinkedInterval($[b,e)$,$[b',e')$, $B$, $C$ $\Pi$, $F_e$, $\mathcal{P}$)}
...
\Input{$[b,e)$, a prefix-interval on a BWT $B$,\\$n$, the prefix-interval dimension,\\$\mathcal{E}_e$, the extension pair files for the next iteration \\$B$, the BWT of a collection of strings\\$C$, the count vector\\$\Pi$, a vector of $|\Sigma|$ integers}
...
\tcp{Check if $P$ is an overlap}
\If{ $\pi[ 0 ] > 0$ } {
  $b'_p \gets \Pi[ 0 ]$\;
  $e'_p \gets \Pi[ 0 ] + \pi[ 0 ]$\;
  \ForEach{$i \gets 1$ to $|\Sigma^\$|$}{
    \If{ $\pi[ i ] > 0$} {
      $b'_e \gets C[ i ] + \Pi[ i ]$\;
      $e'_e \gets b'_e + \pi[ i ]$\;
      Append $\langle [b'_e, e'_e ), [b'_p, e'_p), 1 \rangle$ to $\mathcal{E}_e(\sigma_i)$
    }
  }
}
$\Pi \gets \Pi + \pi$

\caption{processPInterval($[b', e')$, $n$ $B$, $C$, $\mathcal{E}_e$ $\Pi$)}
...
\Input{$[b'_p,e'_p)$, $[b'_s, e'_s)$, $l$ an extension pair on a BWT $B$,\\$\mathcal{E}_e$, the extension pair files for the next iteration \\$B$, the BWT of a collection of strings\\$C$, the count vector for $B$\\$\Pi$, a vector of $|\Sigma|$ integers}

Update $\Pi$ such that $\Pi[i]$ is the number of $\sigma_i$ in $B[0, b')$\;
Create a vector $\pi$ of $|\Sigma^\$|$ integers \st $\pi[i]$ is the number of $\sigma_i$ in $B[b'_p, e'_p)$\;
\tcp{Check if the extension pair is terminal}
\If{ $\pi[ 0 ] > 0$ } {
      $b'_p \gets \Pi[ 0 ]$\;
      $e'_p \gets b'_e + \pi[ i ]$\;
      \Return $\langle [b'_p, e'_p), [b'_e, e'_e), l \rangle$\;
  }
\Else {
  \ForEach{ $i \gets 1 $ to $|\mathcal{E}^\$|$ } {
    $b'_p \gets C[ i ] + \Pi[ i ]$\;
    $e'_p \gets b'_p + \pi[ i ]$\;
    Append $\langle [b'_p, e'_p), [b'_e, e'_e), l+1 \rangle$ to $\mathcal{E}_e(\sigma_i)$\;
  }
}

$\Pi \gets \Pi + \pi$

\caption{processExtPair($[b'_p, e'_p)$, $[b'_s, e'_s)$, $l$, $B$, $C$, $\mathcal{E}_e$ $\Pi$)}
...
>>>>>>> ed65da8... rivista introduzione e conclusioni
