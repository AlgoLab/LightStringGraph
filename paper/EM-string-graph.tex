...
\notaestesa{altri rif a BWT in memoria esterna sono Ferragina2012,
Bauer2013}.
...
Observe that each edge $(r, r_e)$ in a String graph over $R$ is
associated to a terminal extension pair as follows:
...
The main step of our algorithm consists in building efficiently using
external memory the $Q$-intervals on $B$ and the $rev(Q)$-intervals on
$B'$ of increasing length $l$, by means of sequential iterations where
the first iteration starts from a minimum value $\tau$ for the length
$l$.
Each iteration increases by one the length $l$, till the maximum length
of a read in $R$ is reached and each iteration uses the main functions
...
both information produce the list of arcs.



Our algorithm is based on the two methods presented in~\cite{Bauer2011,
Cox2012}.
The first one builds the BWT of a collection of strings using $|\Sigma|$
external files, each one containing the BWT segment related to suffixes
...
In the paper we extend the above approach in the procedure
\emph{processJoinedInterval} in order to produce during the same
iteration the sorted list of $Q$-intervals linked to $rev(Q)$-intervals
of length $l$, given the ones of length $l-1$ on the two BWT's $B$ and
$B'$.

More precisely, the procedure works as follows in external memory.
Assume to have the sorted list of $Q$-intervals with the linked
$rev(Q)$-intervals of length $l-1$ partitioned in $|\Sigma|$ external
files, each one containing the intervals related to a string starting
with the same symbol $\sigma$.
Observe that once such list is given, since the BWT $B$ correspond to a
lexicographic ordering of all suffixes, then when reading $B$ from left
to right, it is partitioned into four $\sigma Q$-intervals corresponding
...
Let us now detail the main procedure \emph{buildGraph$(G, \tau)$} that
builds the arc-set of the string-graph.
Based upon the procedure \emph{processJoinedInterval}, we give two main
procedures that give the iteration step of \emph{buildGraph$(G, \tau)$}
we detail below.
Observe that we describe the algorithm as building three main lists,
that are actually implemented by files that are read once from left to
right.
...
\begin{itemize}
\item $\mathcal{Q}_j$, containing the $Q$-intervals $[b, e)$ on $B$ of
  width $>1$ and length $(j+\tau)$, each one with its linked
...
\item $\mathcal{T}$, containing the $rev(Q)$-intervals $[b', e')$ of
  ...
  $n$.
\item $\mathcal{E}_j$, containing the extension pairs $(q'', q')$ of
  dimension $(j+\tau)$, each one with its extension $e$ $> 0$
\end{itemize}

Observe that the $j$-iteration will produce  $\mathcal{Q}_{j+1}$,
$\mathcal{E}_{j+1}$ that will be used in the next $j+1$-iteration.
...
$(j+\tau-1)$ to produce the intervals in $Q_j$) by using the formula in
(??).
...
$j$-th iteration.
Otherwise (if $q''$ has no extension with symbol $\$$), an extension
pair $(q_e, q')$ (of dimension $(j+\tau+1)$ and extension $e+1$) is
added to the list $\mathcal{E}_{j+1}$, for each $q_e \in X(q'')$, where
$X(q'')$ is the set of the intervals obtained by backward extending
$q''$ with each $\sigma \in \Sigma$.
...
start of the interval $q''$ on the BWT $B'$.
