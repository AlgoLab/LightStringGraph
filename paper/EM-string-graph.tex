...
In fact an arc $(r_{1}, r_{2})$ is called \emph{transitive} (or reducible) if there exists a
...
of the BWT of the reads, the input reads \notaestesa{GDV}{ Marco, mi confermi
  che non si riesce a tenere in ram solo una porzione?} and some auxiliary arrays.
...
The arrays that constitute the FM-index are defined as usual: $C(\sigma,
R)$ denotes the number of symbols in $R$ smaller than the symbol
...
The two arrays $C(\sigma, R)$ and $Occ(\sigma, i, R)$, together with the
string $B$,
\YP{$B$ è richiesta?}
...
$\mathcal{T}\gets$ create an empty file\;
  $extendQOnLeft(\mathcal{Q}_j, B, C, \mathcal{Q}_{j+1}, \mathcal{T})$\;
  sort $\mathcal{T}$\;
  $\mathcal{E}_{j+1} \gets extendQOnRight(\mathcal{T}, \mathcal{E}_j, B', C)$\;
  $j \gets j+1$\;
}
...
Initialize a vector $\Pi \gets [0, 0, . . . , 0]$ of $|\Sigma^\$|$ counters\;
 ...
 \For{$i \gets 0$ \KwTo  $|\Sigma|$}{
	...
	$processLinkedInterval([b, e), [b', e'), B, C, \Pi, F_e, \mathcal{T})$\;
% \tcp{check if $[b',e')$ is a $P$-interval}
% 	\If{$[b,e)$ has width $>1$ and $n >0$ $\$$-extensions}{
%           Append $\langle [b', e'), n\rangle$ to file $\mathcal{T}$\;
% 	}
     }
}

\caption{extendQOnLeft($F$, $B$, $C$, $F_e$, $\mathcal{T}$)}
...
\begin{algorithm}
\SetKwInOut{Input}{Input}\SetKwInOut{Output}{Output}
\Input{$\mathcal{T}$, the sorted list of the $P$-intervals of a given length $l$,\\$\mathcal{E}$, the sorted list of the extension pairs of dimension $l$,\\$B'$, the BWT of the reversed set $R'$, \\$C$, count vector for $B'$}
...
Initialize vector $\Pi \gets [0, 0, \ldots , 0]$ of $|\Sigma^\$|$ integers\;
    ...
    $\langle [b', e'), n \rangle \gets$ getFirst$(\mathcal{T})$\;
...
\caption{extendQOnRight($\mathcal{T}$, $\mathcal{E}$, $B'$, $C$)}
...
%   $b_e \gets |R|+C(\sigma_i, R)+ \Pi[i]$\;
   \If{ $\pi[i] > 0$ } {
     \If{ $i = 0$ } {
       \tcp{P-Interval}
       Append $\langle [b'_e, e'_e), \pi[i] \rangle$ to $\mathcal{T}$\;
     }
     \Else {
       $b_e \gets C[ i ] + \Pi[i]$\;
       $e_e \gets b_e + \pi[i]$\;
...
\caption{processLinkedInterval($[b,e)$,$[b',e')$, $B$, $C$ $\Pi$, $F_e$, $\mathcal{T}$)}
...
\Input{$[b,e)$, a $P$-interval on a BWT $B$,\\$n$, the $P$-interval dimension,\\$\mathcal{E}_e$, the extension pair files for the next iteration \\$B$, the BWT of a collection of strings\\$C$, the count vector\\$\Pi$, a vector of $|\Sigma|$ integers}
